...
if %E_Debug%==true (
	core\sfk.exe echo "!E_DebugColour_Normal![%TIME%] [DMCNet] Parsing class to processing"
)

:class.process

if !Function_%className%.main.declared!==true (
	set %className%.classDefined=true
						
	call :function.invoke "%className%.main"
...
:readInput 
if %E_Debug%==true (
	core\sfk.exe echo "!E_DebugColour_Normal![%TIME%] [DMCNet] Parsing string '%input%' to processing"
)
...
for /f "%~1 tokens=1* delims=%~2" %%a in ("%~3") do (
	...
	if defined classBlockDefined (
		...
		if "!fchar!"=="$" (
			...
			if /i "!val!"=="Abstract" (
				set Class_%className%.abstract=true
			)
		)
...
if %function%==line (
	...
	if not %Mode%==ignorePrompt (
		echo.
		core\sfk.exe echo "!E_DebugColour_Normal![%TIME%] [DMCNet] Class has ended"
		core\sfk.exe echo "!E_DebugColour_Normal!"[%TIME%] [DMCNet] Press any key to close the console window...
		pause>nul
	)
